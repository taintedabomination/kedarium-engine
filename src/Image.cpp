...
const bool kdr::Image::loadFromPNG(std::string pngPath, unsigned char** data, int& width, int& height)
{
  png_image image;
  memset(&image, 0, sizeof(image));
  image.version = PNG_IMAGE_VERSION;

  if (!png_image_begin_read_from_file(&image, pngPath.c_str())) {
    std::cerr << "Failed to begin reading PNG: " << pngPath << std::endl;
    return false;
  }
  ...
  *data = new unsigned char[PNG_IMAGE_SIZE(image)];
  ...
  if (!png_image_finish_read(&image, nullptr, *data, 0, nullptr)) {
    std::cerr << "Failed to finish reading PNG: " << pngPath << std::endl;
    delete[] *data;
    return false;
  }
